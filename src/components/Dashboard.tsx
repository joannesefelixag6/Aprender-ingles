      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm">
             <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-xl font-bold font-display">Activity Progress</h3>
                  <p className="text-sm text-slate-500">Your XP earnings this week</p>
                </div>
                <div className="text-right">
                  <span className="text-2xl font-black text-indigo-600">+1,820</span>
                  ...
                  <BarChart data={data}>
                    ...
                    <Bar dataKey="xp" radius={[6, 6, 0, 0]} barSize={32}>
                      {data.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={index === data.length - 2 ? '#4F46E5' : '#E2E8F0'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <div className="space-y-8">
           <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm h-full">
              <h3 className="text-xl font-bold font-display mb-6">Your Progress</h3>
              
              <div className="space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-bold text-slate-600">Level {stats.level} Intermediate</span>
                ...
                <div className="pt-4 border-t border-slate-100 space-y-4">
                  <h4 className="text-sm font-bold text-slate-900">Today's Goals</h4>
...
function PracticeCard({ icon: Icon, title, desc, color, onClick }: any) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`p-6 rounded-3xl border text-left transition-all hover:shadow-xl text-indigo-600 group w-full bg-white`}
    >
      ...
      <h3 className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight text-xs mb-1">
        {title}
      </h3>
...
function StatItem({ label, value, suffix = '' }: any) {
  return (
    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
      ...
      <p className="text-xl font-black text-slate-800">{value} <span className="text-xs text-slate-400 font-medium">{suffix}</span></p>
    </div>
  );
}
